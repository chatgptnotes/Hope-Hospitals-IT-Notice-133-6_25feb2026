...
export default function TransactionTable({ transactions, onEdit, onDelete }: Props) {
  ...
  // Extract patient name from narration (format: "PatientName: Diagnosis")
  const getPatientName = (narration: string) => {
    const colonIdx = narration.indexOf(':');
    return colonIdx > 0 ? narration.substring(0, colonIdx).trim() : narration;
  };
    ...
    <div className="bg-white rounded-lg border shadow-sm overflow-x-auto">
      <table className="min-w-full text-sm">
        <thead className="bg-blue-900 text-white">
          <tr>
            {[
              'Sr.No','Transaction Date','Patient Name','Patient Visit ID','Admission ID',
              'Voucher No','Invoice Amount','Discount Amount','Net Bill Amount',
              'Cash Amount','Other Mode Payment','Transaction Type','Department',
              'Treatment Code','Narration','Actions'
            ].map(h => (
        ...
        <tbody>
          {transactions.map((t, i) => (
            <tr key={t.id ?? i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
              ...
              <td className="px-3 py-2 whitespace-nowrap font-medium text-gray-800">{getPatientName(t.narration)}</td>
              ...
              <td className="px-3 py-2 max-w-[200px] truncate text-xs" title={t.treatment_code}>{t.treatment_code || '—'}</td>
